...
% Paper lift coefficients
liftCoeff = 0.1083*(foilAngleOfAttack*180/pi) - 0.0002;
dragCoeff = 6E-05*(foilAngleOfAttack*180/pi)^2 + 5E-07*(foilAngleOfAttack*180/pi) + 0.0051;

if strcmp(foil.type, 'sail')
    if (foilAngleOfAttack*180/pi) < 28
        liftCoeff = -0.000001*(foilAngleOfAttack*180/pi)^2 + 0.0927*(foilAngleOfAttack*180/pi) + 0.0099;
    else
        liftCoeff = -0.0175*(foilAngleOfAttack*180/pi)+ 3;
    end
    dragCoeff = 0.7*(0.00006*(foilAngleOfAttack*180/pi)^2-0.0014*(foilAngleOfAttack*180/pi)+0.0097);
end

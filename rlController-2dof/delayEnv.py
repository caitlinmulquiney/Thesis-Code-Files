...
class HydrofoilEnv(gym.Env):
    def __init__(self, use_m_network: bool = False):
        super().__init__()
        self.delay_steps = 5
    ...
    def step(self, action):
        ...
        survival_bonus = 0 #2.0 * pitch_health * roll_health * height_health

        height_reward = 2-40.0 * height_error**2
        pitch_reward = 1-50 * pitch_error**2
        roll_reward = 1-100 * roll_error**2
        drift_reward = 1-50*drift_error**2
        roll_rate_reward = 1-50*roll_rate_error**2
        yaw_reward = 1-50*yaw**2
        gap_reward = -0.5 * np.sum(delta**2)
        action_reward = 0 #-0.1 * np.sum(action**2)

        reward = height_reward + pitch_reward + roll_reward + yaw_reward + drift_reward + roll_rate_reward + survival_bonus + gap_reward + action_reward
